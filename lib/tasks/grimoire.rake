namespace :grimoire do
  ...
  task :parse => :environment do
    client = BungieClient::Wrapper.new(
      :client => BungieClient::RailsClient.new(
        :ttl => 1.hour,
        :prefix => 'parser'
      )
    )
    ...
    if grimoire&.Response&.themeCollection.nil?
      p 'Wrong grimoire'
    else
      ...
      grimoire.Response.themeCollection.each do |theme|
        ...
        theme.pageCollection.each do |collection|
          ...
          collection.cardCollection.each do |card|
            ...
            report << card_obj.name unless card_obj.persisted?
    ...
    ParserMailer.report(report).deliver_later if report.any?
  end
end
